...
from default_temperature import default_temp
...
def get_cel():
    ...
    return f'{global_temp}{chr(176)}C'


def get_kelvin():

    return f'{round(global_temp+273.15, 2)}{chr(176)}K'


def get_fare():

    return f'{round(global_temp * 9/5 + 32, 2)}{chr(176)}F'




def settings():  # TODO: Settings Button
    class main_settings(object):
        def __init__(self, master):
            self.master = master

            master.title('Settings')
            master.geometry('400x400')
            metric_label = Label(master, text='Units', fg=_from_rgb((66, 176, 244)),
                                 font=('Helvetica', 13))
            metric_label.place(x=25, y=50)
            self.rbv = tk.IntVar(master)
            self.rb1 = ttk.Radiobutton(master, text="Celsius",
                                       variable=self.rbv, value=0, command=self.onRadioChange)
            self.rb1.place(x=100, y=50)
            self.rb2 = ttk.Radiobutton(master, text="Kelvin",
                                       variable=self.rbv, value=1, command=self.onRadioChange)
            self.rb2.place(x=180, y=50)
            self.rb3 = ttk.Radiobutton(master, text="Farenhait",
                                       variable=self.rbv, value=2, command=self.onRadioChange)
            self.rb3.place(x=260, y=50)
    ...
    try:
        ...
        for j in response:
            ...
            if temprature is None:
                ...
                global_temp = round(main['temp'] - 273.15, 2)
            if pressure is None:
        ...
        if default_unit == 1:
            main_canvas.itemconfig(te, text=get_kelvin())
        if default_unit == 2:
            main_canvas.itemconfig(te, text=get_fare())
...
# Settings Button
setting_button = Button(scanvas, bg=_from_rgb((59, 58, 60)), command=settings)
